...
#
# This file is both a command line tool and a plugin for the Uefi Build system.
# To do this the plugin class is only defined if in the build environment
# where the plugin classes are importable.
#
#
try:
    ...
    OVERRIDE_FORMAT_VERSION_1 = (
        1,
        4,
    )  # Version 1: #OVERRIDE : VERSION | PATH_TO_MODULE | HASH | YYYY-MM-DDThh-mm-ss
    OVERRIDE_FORMAT_VERSION_2 = (
        2,
        5,
    )  # Version 2: #OVERRIDE : VERSION | PATH_TO_MODULE | HASH | YYYY-MM-DDThh-mm-ss | GIT_COMMIT
    DEPRECATION_FORMAT_VERSION_1 = (
        1,
        4,
    )  # Version 1: # DEPRECATED : VERSION | PATH_TO_NEW_MODULE_TO_USE | YYYY-MM-DDThh-mm-ss | DEPRECATION_TIMELINE
    FORMAT_VERSIONS = [
        OVERRIDE_FORMAT_VERSION_1,
        OVERRIDE_FORMAT_VERSION_2,
        DEPRECATION_FORMAT_VERSION_1,
    ...
    class OverrideValidation(IUefiBuildPlugin):
        ...
        def do_pre_build(self, thebuilder):
            ...
            logging.info(
                "--------------Override/Deprecation Validation Starting---------------"
            )
            logging.info(
                "---------------------------------------------------------------------"
            )
            ...
            logging.info(
                "--------------Override/Deprecation Validation Finished----------------"
            )
            logging.info(
        ...
        def override_plat_validate(self, thebuilder):
            """Walk through the target inf file in the platform dsc file
            searching for overridevalidation strings."""
            ...
            for file in InfFileList:
                ...
                m_result = self.override_detect_process(
                    thebuilder, fullpath, temp_list, modulenode, status
                )
                ...
                if m_result != self.OverrideResult.OR_ALL_GOOD:
                    if m_result != self.OverrideResult.OR_DSC_INF_NOT_FOUND:
                        result = m_result
                    logging.error(
                        "Override/Deprecation processing error %s in file/dir %s"
                        % (self.OverrideResult.GetErrStr(m_result), file)
                    )
        ...
        def override_detect_process(
            ...
            with open(filepath, "r") as my_file:
                for Line in my_file:
                    ...
                    if (len(CommentLine) != 2) or (
                        (CommentLine[0].strip().lower() != "override")
                        and (CommentLine[0].strip().lower() != "track")
                        and (CommentLine[0].strip().lower() != "deprecated")
                    ):
                        continue
                    ...
                    m_result = self.override_process_line(
                        thebuilder,
                        CommentLine[1],
                        filepath,
                        filelist,
                    ...
                    if tagtype == "override":
                        ...
                        if m_result == self.OverrideResult.OR_TARGET_INF_NOT_FOUND:
                            ...
                            track_fc.append(
                                [lineno, Line, modulenode.reflist[-1].path, False]
                            )
                            logging.info("At Line %d: %s" % (lineno, Line))
            ...
            if trackno != 0 and len(track_nf) == trackno:
                ...
                for lineno, Line in track_nf:
                    logging.error(
                        "Track tag failed to locate target module at Line %d: %s"
                        % (lineno, Line)
                    )
            ...
            if len(track_fc) != 0:
                ...
                for entry in track_fc:
                    for all_good_line in track_ag:
                        if entry[2] == all_good_line:
                            ...
                            break
                if (
                    canceled_cnt != len(track_fc)
                    and result == self.OverrideResult.OR_ALL_GOOD
                ):
                ...
                for lineno, Line, _, canceled in track_fc:
                    if not canceled:
                        logging.error(
                            "Track tag failed to match module hash at Line %d: %s"
                            % (lineno, Line)
                        )

            # Revert this visited indicator after this branch is done searching
        ...
        def override_process_line(
            ...
            try:
                ...
                logging.error(
                    "Inf Override Parse Error, override parameter has invalid version %s"
                    % (OverrideEntry[0])
                )
            ...
            for VERSION_FORMAT in FORMAT_VERSIONS:
                if len(VERSION_FORMAT) < 2:
                    ...
                    continue
                if (
                    EntryVersion == VERSION_FORMAT[0]
                    and len(OverrideEntry) == VERSION_FORMAT[1]
                ):
                    version_match = VERSION_FORMAT
                    break
            if version_match == False:
                logging.error(
                    f"Inf Override Unrecognized Version {EntryVersion} or corrupted format ({len(OverrideEntry)}) in this entry: {filepath}"
                )
            ...
            if version_match[0] == 1:
                if tagtype == "deprecated":
                    return self.deprecation_process_line_with_version1(
                        thebuilder, filepath, OverrideEntry
                    )
                else:
                    return self.override_process_line_with_version1(
                        thebuilder, filelist, OverrideEntry, m_node, status, tagtype
                    )
            elif version_match[0] == 2:
                return self.override_process_line_with_version2(
                    thebuilder, filelist, OverrideEntry, m_node, status, tagtype
                )
            else:
        ...
        def override_process_line_with_version1(
            self, thebuilder, filelist, OverrideEntry, m_node, status, tagtype
        ):
            EntryVersion = 1
            ...
            fullpath = (
                thebuilder.edk2path.GetAbsolutePathOnThisSystemFromEdk2RelativePath(
                    overriddenpath, log_errors=False
                )
            )
            # Search overridden module in workspace
            if fullpath is None:
                logging.info(
                    "Inf Overridden File/Path Not Found in Workspace or Packages_Path: %s"
                    % (overriddenpath)
                )
                result = self.OverrideResult.OR_TARGET_INF_NOT_FOUND
            ...
            try:
                EntryTimestamp = datetime.strptime(
                    OverrideEntry[3].strip(), TIMESTAMP_FORMAT
                )
                EntryTimestamp = EntryTimestamp.replace(tzinfo=timezone.utc)
            except ValueError:
                logging.error(
                    "Inf Override Parse Error, override parameter has invalid timestamp %s"
                    % (OverrideEntry[3].strip())
                )
                result = self.OverrideResult.OR_INVALID_FORMAT
                m_node.status = result
                return result
            ...
            res_tuple = self.override_hash_compare(
                thebuilder, EntryVersion, EntryHash, fullpath
            )
            ...
            overridden_rel_path = self.PathTool.GetEdk2RelativePathFromAbsolutePath(
                fullpath
            )
            ...
            if result == self.OverrideResult.OR_ALL_GOOD:
                ...
                pnt_str = "Inf Override Hash Error: %s, expecting %s, has %s" % (
                    self.OverrideResult.GetErrStr(result),
                    m_node.expect_hash,
                    m_node.entry_hash,
                )
                if tagtype == "override":
            ...
            m_result = self.override_detect_process(
                thebuilder, fullpath, filelist, m_node, status
            )
        ...
        def override_process_line_with_version2(
            self, thebuilder, filelist, OverrideEntry, m_node, status, tagtype
        ):
            """#Version 2: #OVERRIDE : VERSION | PATH_TO_MODULE | HASH | YYYY-MM-DDThh-mm-ss | GIT_COMMIT"""
            GitHash = OverrideEntry[4].strip()
            del OverrideEntry[4]
            result = self.override_process_line_with_version1(
                thebuilder, filelist, OverrideEntry, m_node, status, tagtype
            )
            # if we failed, do a diff of the overridden file (as long as exist) and show the output
            if (
                result != self.OverrideResult.OR_ALL_GOOD
                and result != self.OverrideResult.OR_TARGET_INF_NOT_FOUND
            ):
                overriddenpath = os.path.normpath(OverrideEntry[1].strip()).strip("\\")
                fullpath = (
                    thebuilder.edk2path.GetAbsolutePathOnThisSystemFromEdk2RelativePath(
        ...
        def deprecation_process_line_with_version1(
            self, thebuilder, filepath, DeprecationEntry
        ):
            """
            ...
            if skip_list_str is not None:
                ...
                for skipped_module in skip_list:
                    ...
                    if normalized_file_path.endswith(skipped_module_path):
                        logging.info(
                            "Use of Deprecated module: %s skipped as part of platform skip list",
                            filepath,
                        )
            ...
            deprecation_dt = datetime.strptime(
                DeprecationEntry[2].strip(), TIMESTAMP_FORMAT
            ).replace(tzinfo=timezone.utc) + timedelta(days=deprecation_timeline)
            ...
            if current_dt > deprecation_dt:
                logging.error(
                    "Use of Deprecated module: %s post date of deprecation.", filepath
                )
                if replacement_path is None:
                    logging.error(
                        "Please remove the module (or) Add to DEPRECATED_MODULES_SKIPLIST"
                    )
                else:
                    logging.error(
                        "Replace with: %s (or) Add to DEPRECATED_MODULES_SKIPLIST",
                        replacement_path,
                    )
            ...
            # Module is not deprecated yet
                ...
                if replacement_path is None:
                    logging.warning(
                        "Please remove the module (or) Add to DEPRECATED_MODULES_SKIPLIST before %s",
                        deprecation_dt.strftime(TIMESTAMP_FORMAT),
                    )
                else:
                    logging.warning(
                        "Replace with: %s (or) Add to DEPRECATED_MODULES_SKIPLIST before %s",
                        deprecation_dt.strftime(TIMESTAMP_FORMAT),
                    )
        ...
        def node_dfs(self, thebuilder, node, stack, log):
            ...
            for m_node in node.reflist:
                ...
                if m_node.status == self.OverrideResult.OR_ALL_GOOD:
                    ...
                    log.write(
                        "\t" * list_len
                        + "| \tCurrent State: %s | Last Fingerprint: %s\n"
        ...
        def get_dsc_inf_list(self, thebuilder):
            ...
            # Parse the DSC
            pa = thebuilder.edk2path.GetAbsolutePathOnThisSystemFromEdk2RelativePath(
                plat_dsc
            )
            dscp.ParseFile(pa)
            ...
            if "FLASH_DEFINITION" in dscp.LocalVars:
                fd = (
                    thebuilder.edk2path.GetAbsolutePathOnThisSystemFromEdk2RelativePath(
                        dscp.LocalVars["FLASH_DEFINITION"]
                    )
                )
...
def ModuleHashCal(path: str):
    """Calculates the md5 hash for the path..

    path can correspond to a directory, or a specific file.
    It should be a complete path for the file.

    if its a directory, all the files of the directory will be hashed.
    if its a file, the file itself will be hashed.
    If its an INF file, the file itself, and the items listed in the sources section of the INF
    will be hashed.
    """
    ...
    for sfile in sourcefileList:
        logging.debug(f"Calculated: {sfile}")
    ...
    for bfile in binaryfileList:
        logging.debug(f"Calculated: {bfile}")
...
def ModuleHash2Cal(path: str):
    """Calculates the md5 hash for the path..

    path can correspond to a directory, or a specific file.
    It should be a complete path for the file.

    if its a directory, all the files of the directory will be collected,
    sorted, and the sorted list of the files will be hashed.
    if its a file, the file itself will be hashed.
    If its an INF file, the file itself, and the items listed in the sources
    section of the INF will be hashed.
    """
...
def ModuleGitHash(path):
    ...
    ret = RunCmd(
        ...
        workingdir=abspath_dir,
        outstream=git_stream,
        logging_level=logging.DEBUG,
    )
    if ret != 0:
        return None
...
def path_parse():
    ...
    parser.add_argument(
        "-dr",
        "--deprecationreplacementpath",
        dest="DeprecationReplacementPath",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "-dt",
        "--deprecationtimeline",
        dest="DeprecationTimelineInDays",
        type=int,
        default=90,
    ...
    if Paths.TargetPath is not None:
        ...
        if not os.path.normcase(Paths.TargetPath).startswith(
            os.path.normcase(Paths.WorkSpace.rstrip(os.sep)) + os.sep
        ):
    ...
    if Paths.RegenPath is not None:
        ...
        # For a folder, this will do nothing on a formatted abspath
        # For a drive root, this will rip off the os.sep
        if not os.path.normcase(Paths.RegenPath).startswith(
            os.path.normcase(Paths.WorkSpace.rstrip(os.sep)) + os.sep
        ):
            raise RuntimeError("Module is not within specified Workspace.")
    ...
    if Paths.DeprecatedPath is not None:
        if not os.path.isfile(Paths.DeprecatedPath):
            raise RuntimeError(
                f"Deprecation path {Paths.DeprecatedPath} module is invalid"
            )
        # Needs to strip os.sep is to take care of the root path case
        # For a folder, this will do nothing on a formatted abspath
        # For a drive root, this will rip off the os.sep
        if not os.path.normcase(Paths.DeprecatedPath).startswith(
            os.path.normcase(Paths.WorkSpace.rstrip(os.sep)) + os.sep
        ):
            raise RuntimeError(
                f"Module {Paths.DeprecatedPath} is not within the specified Workspace {Paths.WorkSpace.rstrip(os.sep)}."
            )
    ...
    if Paths.RegenPath is not None:
        ...
        for line in RegenInfData.splitlines(True):
            ...
            if match is not None:
                ...
                abs_path = pathtool.GetAbsolutePathOnThisSystemFromEdk2RelativePath(
                    rel_path
                )
                if abs_path is not None:
                    ...
                    if mod_hash != match.group(4) and mod_hash2 != match.group(4):
                        ...
                        if VERSION_INDEX == 0:
                            line = f"#{match.group(1)} : {OVERRIDE_FORMAT_VERSION_1[0]:08d} | {rel_path} | {mod_hash2} | {datetime.now(timezone.utc).strftime(TIMESTAMP_FORMAT)}\n"

                        elif VERSION_INDEX == 1:
                            git_hash = ModuleGitHash(abs_path)
                            line = f"#{match.group(1)} : {OVERRIDE_FORMAT_VERSION_2[0]:08d} | {rel_path} | {mod_hash2} | {datetime.now(timezone.utc).strftime(TIMESTAMP_FORMAT)} | {git_hash}\n"
                        print("Updating:\n" + line)
                else:
                    print(
                        f"Warning: Could not resolve relative path {rel_path}. Override line not updated.\n"
                    )
        ...
        if Paths.DeprecatedPath is not None:
            ...
            if Paths.DeprecationReplacementPath is not None:
                pkg_path = pathtool.GetContainingPackage(
                    Paths.DeprecationReplacementPath
                )
                if pkg_path is not None:
                    rel_path = Paths.DeprecationReplacementPath[
                        Paths.DeprecationReplacementPath.find(pkg_path) :
                    ]
                else:
                    rel_path = pathtool.GetEdk2RelativePathFromAbsolutePath(
                        Paths.DeprecationReplacementPath
                    )
                    if not rel_path:
                        print(
                            f"{Paths.DeprecationReplacementPath} is invalid for this workspace."
                        )
                        sys.exit(1)
            ...
            print(
                f"The module will be deprecated in {Paths.DeprecationTimelineInDays} days"
            )
            print(
                "Copy and paste the following line(s) to your deprecated inf file(s):\n"
            )
            print(
                "#%s : %08d | %s | %s | %s\n"
                % (
                    "Deprecated",
                    DEPRECATION_FORMAT_VERSION_1[0],
                    rel_path,
                    datetime.now(timezone.utc).strftime(TIMESTAMP_FORMAT),
                    str(Paths.DeprecationTimelineInDays),
                )
            )
            ...
            if VERSION_INDEX == 0:
                ...
                print(
                    "#%s : %08d | %s | %s | %s"
                    % (
                        "Override" if not Paths.Track else "Track",
                        OVERRIDE_FORMAT_VERSION_1[0],
                        rel_path,
                        mod_hash,
                        datetime.now(timezone.utc).strftime(TIMESTAMP_FORMAT),
                    )
